"""Core implementation for the YAML-configured LLM inference runner.

All application-specific choices live in a YAML file and Markdown/JSON assets.
The runner validates model responses and writes durable Parquet result files;
evaluation metrics are intentionally outside its scope.
"""
...
UNRESOLVED_ENVIRONMENT_VARIABLE = re.compile(r"\$(?:\{[A-Za-z_][A-Za-z0-9_]*\}|[A-Za-z_][A-Za-z0-9_]*)")
...
def resolve(config: dict[str, Any], value: str | Path) -> Path:
    ...
    unresolved = UNRESOLVED_ENVIRONMENT_VARIABLE.search(expanded)
    if unresolved:
        raise ValueError(f"Environment variable is not set: {unresolved.group(0)}")
    path = Path(expanded)
...
def _slugify_label(label: str) -> str:
    """Lowercase, non-alphanumeric runs collapsed to one underscore, edges stripped."""
    return re.sub(r"[^a-z0-9]+", "_", label.lower()).strip("_")


def _normalize_gold_labels(raw_labels: list[str], canonical: list[str] | None) -> list[str]:
    """Some CSV sources store gold labels as raw category identifiers (e.g.
    ProtoEthos's `value_id` column: "benevolence_caring") that don't match
    the theory's canonical, human-readable label strings used everywhere
    else for that same theory ("Benevolence: caring") -- confirmed for
    `protoethos_shvt` via a direct row audit, 2026-07-18: all 20 SHVT
    `value_id` values slugify cleanly onto their canonical label, while the
    model is prompted with (and its predictions validated against) the
    canonical form, so a raw, unnormalized gold_labels value would silently
    read as 0% accuracy even for a perfectly correct model. Map any raw
    label that isn't already an exact canonical match through a slug
    lookup; a raw label with no slug match is left as-is so a genuinely
    wrong gold value stays visible instead of being silently dropped.
    """
    if not canonical:
        return raw_labels
    canonical_set = set(canonical)
    lookup = {_slugify_label(label): label for label in canonical}
    return [label if label in canonical_set else lookup.get(_slugify_label(label), label) for label in raw_labels]
...
def read_rows(
    ...
    for position, row in enumerate(rows):
        ...
        if "_gold_labels" not in row and source.get("labels_column"):
            row["_gold_labels"] = _normalize_gold_labels(
                _split_labels(row.get(source["labels_column"])), source.get("labels")
            )
    ...
    with path.open(encoding="utf-8", newline="") as handle:
        for raw in csv.DictReader(handle, delimiter=delimiter):
            ...
            if resolved_source.get("labels_column") and "_gold_labels" not in row:
                row["_gold_labels"] = _normalize_gold_labels(
                    _split_labels(row.get(resolved_source["labels_column"])), resolved_source.get("labels")
                )
...
class Backend(Protocol):
    ...
    vLLM one (VLLMBackend); an OpenAI-compatible endpoint and fake are the alternates."""
...
class OpenAICompatibleBackend:
    """Vendor-neutral OpenAI-compatible endpoint with bounded HTTP retries."""
...
def run(config: dict[str, Any], backend: Backend | None = None, row_limit: int | None = None) -> dict[str, Any]:
    ...
    try:
        for configured_variant in config["variants"]:
            ...
            expand_labels = variant_expansion_labels(config, configured_variant)
            ...
            if materialised is not None:
                ...
                if not input_counted:
                    # Count source inputs, not expanded requests: `input_rows` in
                    # the manifest is the number of items, so counting wraps the
                    # base iterator before the per-label fan-out below.
                    source_iter = _counted(source_iter, pulled)
            if expand_labels is not None:
                source_iter = expanded_rows(source_iter, expand_labels)
            ...
            if materialised is not None:
                counted_rows = (
                    expanded_rows(iter(materialised), expand_labels) if expand_labels is not None else materialised
                )
                pending_rows = sum(
                    1
...
def prepare(config: dict[str, Any]) -> Path:
    ...
    with path.open("w", encoding="utf-8") as handle:
        for variant in config["variants"]:
            schema = variant_schema(config, variant)
            labels = variant_expansion_labels(config, variant)
            source = iter_rows_for_source(config, config["input"])
            variant_rows = expanded_rows(source, labels) if labels is not None else source
...
def variant_expansion_labels(config: dict[str, Any], variant: dict[str, Any]) -> list[str] | None:
    """Labels to fan each source row out over, or None when the variant doesn't.

    `expand_over: dataset_labels` turns one item into one request per label: the
    label-wise yes/no probe a soft multi-label profile needs. Any other value is
    rejected so a typo can't silently collapse the fan-out to a single request.
    """
    spec = variant.get("expand_over")
    if not spec:
        return None
    if spec != "dataset_labels":
        raise ValueError(f"{variant.get('id')}: unsupported expand_over {spec!r} (only dataset_labels)")
    return list(config.get("run", {}).get("dataset_labels", []))


def expanded_rows(rows: Any, labels: list[str]) -> Any:
    """Yield one clone of each row per label, with a stable, unique identity.

    `input_row_id` is preserved so a full item can be reassembled downstream; the
    label lands in `_target_label` and `_source_position` is offset per label so
    the resume key `(variant_id, input_row_id, source_position)` stays unique and
    deterministic across resumes. An interrupted item therefore never looks
    complete with labels missing: every (item, label) pair is its own resumable
    unit.
    """
    width = len(labels)
    for row in rows:
        ...
        for label_index, label in enumerate(labels):
            yield {**row, "_target_label": label, "_source_position": base * width + label_index}
...
def build_requests(config: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for variant in config["variants"]:
        schema = variant_schema(config, variant)
        labels = variant_expansion_labels(config, variant)
        variant_rows = expanded_rows(rows, labels) if labels is not None else rows
        requests.extend(request_for_row(config, variant, row, schema) for row in variant_rows)
    return requests
...
def main() -> int:
    ...
    for name in ("validate", "run", "run-matrix", "prepare", "prepare-matrix", "batch-command", "self-test"):
        ...
        if name in {"run", "run-matrix", "prepare", "prepare-matrix"}:
            command.add_argument("--variants", help="Comma-separated variant ids to run (default: all)")
    ...
    if getattr(args, "variants", None):
        ...
        config["variants"] = [variant for variant in config["variants"] if variant["id"] in set(wanted)]
